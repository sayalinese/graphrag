...
def smart_split_md_html(documents: List[Document], recursive_splitter: RecursiveCharacterTextSplitter | None = None) -> List[Document]:
    ...
    for doc in documents:
        ...
        try:
            ...
            # 对过长段落做二次细分
            for sd in enriched:
                if len(sd.page_content) > 1200:
                    # 保留元数据进行再切分
                    parts = rec.split_text(sd.page_content)
                    for p in parts:
                        out.append(Document(page_content=p, metadata=dict(sd.metadata)))
                else:
                    out.append(sd)
        except Exception:
            # 任意异常则回退（由调用方继续默认切分）
...
def semantic_refine_with_embeddings(
    docs: _List[Document],
    embedding_service,  # KBEmbeddingService 或有 embeddings 属性的对象
