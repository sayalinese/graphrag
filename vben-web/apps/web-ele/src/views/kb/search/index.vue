...
const handleSearch = async () => {
  ...
  try {
    ...
    if (data.length === 0) {
      ...
      ElMessage.success(`找到 ${data.length} 条结果，耗时 ${searchTime.value}ms`);
    }
  } catch (error: any) {
        ...
        <el-col :xs="24" :sm="24" :md="16" :lg="18">
          ...
          <div v-else-if="results.length > 0" class="space-y-4">
            ...
            >
              ...
              <div class="mt-3 pt-3 border-t border-dashed border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-between gap-2 text-xs text-gray-500">
          ...
          <el-empty v-else description="暂无搜索结果，请在上方输入问题进行检索" />
        </el-col>
      </el-row>
    </div>
  </Page>
